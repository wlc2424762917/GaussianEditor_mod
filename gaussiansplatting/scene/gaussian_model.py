...
class GaussianModel:
    ...
    def _prune_optimizer(self, mask):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            stored_state = self.optimizer.state.get(group["params"][0], None)  # 对应param的优化状态
            # 有优化状态的参数，需要将优化状态也进行裁剪
    ...
    def prune_points(self, mask):
        ...
        optimizable_tensors = self._prune_optimizer(valid_points_mask)  # 裁剪优化器参数

        # 保留mask为1的点, 得到新的点云及其基础属性
